...
def drawRectangle(isAvailable, img, a, b, c, d):
...
def drawText(isAvailable, img, text, pos):
    if isAvailable:
        cv2.putText(img, 'Lot ' + text + ' is Available', pos, cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 3)
    else:
        cv2.putText(img, 'Lot ' + text + ' is not Available', pos, cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 3)


def checkisAvailable(img, a, b, c, d):
...
text = [str(i+1) for i in range(len(rois))]
# creating the parameters window with trackbars
cv2.namedWindow('parameters')
cv2.resizeWindow('parameters', 800, 800)
...
while True:
    ...
    for i in range(len(rois)):
        isAvailable = checkisAvailable(frame, rois[i][0], rois[i][1], rois[i][2], rois[i][3])
        drawRectangle(isAvailable,frame, rois[i][0], rois[i][1], rois[i][2], rois[i][3])
        drawText(isAvailable,bg, text[i], (10, 60+i*30))

    # adding the number of available spots on the shown image
    font = cv2.FONT_HERSHEY_SIMPLEX
    cv2.putText(bg, 'Available spots:  ' + str(spots.loc),
